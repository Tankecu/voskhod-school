/* ═══════════════════════════════════════════════════════════
   ...
   Мобильная экономия: пониженное разрешение, ~30 fps,
   упрощённая туманность (3 октавы, без искажения),
   авто-деградация до статичного кадра на слабых устройствах,
   заморозка фона на время экзамена (window.__cosmosFrozen).
   ═══════════════════════════════════════════════════════════ */
...
(function () {
  ...
  /* LOW-версия для слабых устройств: туманность без доменного
     искажения и с 3 октавами — втрое меньше работы на пиксель */
  function makeFrag(low) {
    var oct = low ? 3 : 5;
    var warp = low
      ? '  float q = fbm(np);'
  ...
  var fs = compile(gl.FRAGMENT_SHADER, makeFrag(isMobile));
  ...
  var DPR_CAP = isMobile ? 1.0 : 1.75;  /* мобилка: звёздам не нужен retina */
  var FRAME_MIN = isMobile ? 33 : 16;   /* мобилка: ~30 fps за глаза */
  ...
  /* авто-деградация: замер fps, максимум два шага вниз
     [1] DPR=1 (моб: 0.7) → [2] статичный кадр, цикл остановлен */
  var degradeStep = 0;
  var frames = 0, fpsT0 = null;
  ...
  function frame(now) {
    ...
    if (paused) return;
    if (window.__cosmosFrozen) return; /* экзамен: фон заморожен */

    if (now - lastDraw < FRAME_MIN - 2) return;
    lastDraw = now;

    frames++;
    if (fpsT0 === null) fpsT0 = now;

    if (frames >= 50 && degradeStep < 2) {
      var fps = frames / ((now - fpsT0) / 1000);
      frames = 0;
      fpsT0 = now;
      var cap = isMobile ? 24 : 45;
      if (fps < cap) {
        degradeStep++;
        if (degradeStep === 1) {
          DPR_CAP = isMobile ? 0.7 : 1.0;
          resize();
          console.info('cosmos: понижено разрешение фона (fps ' + fps.toFixed(0) + ')');
        } else {
          draw((now - t0) / 1000, mouse.x, mouse.y);
          console.info('cosmos: фон заморожен в статичный кадр (fps ' + fps.toFixed(0) + ')');
